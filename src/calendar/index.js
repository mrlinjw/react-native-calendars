...
class Calendar extends Component {
  ...
  pressDay(day, dayMarked) {
    const minDate = parseDate(this.props.minDate);
    const maxDate = parseDate(this.props.maxDate);
    if (!(minDate && !dateutils.isGTE(day, minDate)) && !(maxDate && !dateutils.isLTE(day, maxDate))) {
      this.updateMonth(day);
      if (this.props.onDayPress) {
        this.props.onDayPress(xdateToData(day), dayMarked);
      }
    }
  }
  ...
  renderDay(day, id) {
    ...
    if (!dateutils.sameMonth(day, this.state.currentMonth) && this.props.hideExtraDays) {
      ...
      let dayMarked = this.getDateMarking(day);
      let ouputDayMarked = {};
      if(dayMarked != false && !(dayMarked instanceof Array)){
        ouputDayMarked = dayMarked;
      }
      dayComp = (
        <DayComp
            key={id}
            ...
            onPress={this.pressDay.bind(this, day, ouputDayMarked)}
            marked={dayMarked}
  ...
  renderWeek(days, id) {
    ...
    return (<View style={[this.style.week, this.props.weekLine ]} key={id}>{week}</View>);
  }
